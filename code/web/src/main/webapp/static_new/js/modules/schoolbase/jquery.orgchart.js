...
(function(factory) {
  ...
  function createNode(nodeData, level, opts) {
    ...
    $nodeDiv.on('click','.sp2',function(event){
      var tp = $(this).parent();
      var b = tp.next().text();
      $(this).parent().parent().prepend('<input type="text" class="inputs" value='+b+'>');
      var a = tp.prev();
      $('.opti').remove();
      a.focus();
      return false;
    });
    //$nodeDiv.on('blur','input',function(event){
    //  var nn = $(this).next();
    //  var val = $(this).val();
    //  nn.text(val)
    //  $('.node input ').remove();
