...
<div class="right-pos">
  <c:if test="${roles:isManager(sessionValue.userRole) ||roles:isHeadmaster(sessionValue.userRole)||roles:isTeacher(sessionValue.userRole)}">
   ...
   <a href="/newisolatepage/allTable.do" class="pka1">总课表</a>
    ...
    <a href="/newisolatepage/jSubCLassTime.do" class="pkb2">行政班课表</a>
  </c:if>
  <c:if test="${roles:isStudent(sessionValue.userRole)}">
    <a href="/newisolatepage/pStuClassTimeStu.do" class="pkc1">学生课表</a>
    <%--<a href="/newisolatepage/jSubCLassTime.do" class="pkb2">行政班课表</a>--%>
  </c:if>
  <c:if test="${roles:isManager(sessionValue.userRole) ||roles:isHeadmaster(sessionValue.userRole)}">
...
<div class="container clearfix">
  ...
  <script type="text/template" id="grade_temp">
    {{~it:value:index}}
    <label ids="{{=value.gid}}" gnm='{{=value.gnm}}' gJie='{{=value.jie}}'>{{=value.gnm}}({{=value.jie}})</label>
    {{~}}
  </script>
  <div class="container clearfix">
    <div class="fl w225">
      <div class="f18 c-b">班级列表<em class="c-3c">（班级数：<em class="clsSize"></em>）</em></div>
      <ul class="t-left h588 mt10" id="Cls">

      </ul>
      <script type="text/template" id="class_temp">
        {{~it:value:index}}
        <li  ids="{{=value.classId}}">
          <span>{{=value.gname}}（{{=value.xh}}）班</span>
        </li>
        {{~}}
      </script>
    </div>
    <div class="fr w940 mr10" id="GDGContent">
      <div class = "dc">
        <div class="f18 c-b"><em id="ban" style="margin-right: 10px;">高一（1）班</em>
            <label><input type="checkbox" id="xq">查看详情</label>
        </div>
        <button class="b9 fr mtt41">导出</button>
        <button class="b10 fr mtt41 mr85">导出全部课表</button>
        <button class="b8 fr mtt41 mr226" id="printGDG">打印</button>
        <select class="sel1 fr mtt41 mr285" type="text" id="week">
        </select>
...
<div class="bg"></div>
<div class="popup jsupopup">
    <div class="popup-top">
        <em class="fl" id="popup-topName"></em>
        <i class="close fr"></i>
    </div>
    <div class="clearfix jsuSp mtl10">
      <script type="text/template" id="jxbListFo_temp">
        {{~it:value:index}}
        <span class="spl" ids="{{=value.id}}">{{=value.name}}</span>
        {{~}}
      </script>
        <div class="fl w495" id="jxbListFo">
            <span class="spCur alSp">全部</span>
        </div>
        <button class="fr dcBtn">导出</button>
    </div>
    <div class="tabJst">
        <table>
            <thead>
                <tr>
                    <th class="w110">姓名</th>
                    <th class="w50">层级</th>
                    <th class="w145">学籍号</th>
                    <th class="w100">性别</th>
                    <th class="w135">教学班</th>
                </tr>
            </thead>
        </table>
        <div class="w615 ha360">
            <table>
              <script type="text/template" id="stuBody_temp">
                {{~it:value:index}}
                <tr>
                  <td class="w110">{{=value.userName}}</td>
                  {{? value.level == 0}}
                  <td class="w50">无</td>
                  {{?}}
                  {{? value.level != 0}}
                  <td class="w50">{{=value.level}}</td>
                  {{?}}
                  <td class="w145">{{=value.studyNumber}}</td>
                  <td class="w100">{{=value.sexStr}}</td>
                  <td class="w135">{{=value.jxbName}}</td>
                </tr>
                {{~}}
              </script>
                <tbody id="stuBody">
...
<script>
  seajs.use('jSubCLassTime', function (jSubCLassTime) {
    jSubCLassTime.init();
  });
</script>
</html>