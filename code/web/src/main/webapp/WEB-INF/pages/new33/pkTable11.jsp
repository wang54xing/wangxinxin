...
<head>
    ...
    .plDDv>div>button:hover{
        background: #1ab02f!important;
        color: #fff!important;
    }
    .plDDv:hover button{
        display: inline-block;
    }
    .plDDv>div:hover button{
        display: inline-block;
    }
    </style>
...
<div class="fixedcont" id="move" style="width: 130px;">
    <div id="movebg" class="h0">点击可移动</div>
    <div class="info">
        <p class="p1"><label style="float: left">待排教学班</label>
            <label style="font-size: 15px;display: inline-block;float: left">(<i class="ypcount"></i>/<i class="kpcount"></i>)</label>
            <em>×</em>
        </p>
        <p class="p2">
            <select id="jxbType">
    ...
    <div class="cont clearfix h0">
        <div class="lt">
            <%--<span class="active" id = "tagByTeaID">学<br>科</span>--%>
            <%--<span>标<br>签</span>--%>
        </div>
        <ul class="ul2 subject" style="display:none;">
        ...
        <ul class="ul1 kpjxblist cx" style="width: 180px;">

        </ul>
        <script type="text/template" id="kpjxblistTemp">
            {{~it:value:index}}
            {{?value.ypksCount<value.zksCount}}
            <li draggable="false" ondrop="drop(event,this)" ondragover="allowDrop(event)"
                ondragstart="drag(event, this)" clsrmId="{{=value.classroomId}}" tid="{{=value.teacherId}}"
                jxbType="{{=value.type}}" jxbId="{{=value.id}}" ypksCnt="{{=value.ypksCount}}"
                zksCnt="{{=value.zksCount}}">
                <div class="ls">
                    <p><em class="vxk">
                        {{?value.nickName == ""}}
                        {{=value.name}}
                        ...
                        {{=value.nickName}}
                        {{?}}
                        </em><em class="vrs">({{=value.studentCount}}人)</em>
                    </p>

                    {{?value.danOrShuang == 1}}
                    <p class="vjs" js='{{=value.teacherId}}'>{{=value.teacherName}}(单周)</p>
                    {{?}}
                    {{?value.danOrShuang == 2}}
                    <p class="vjs" js='{{=value.teacherId}}'>{{=value.teacherName}}(双周)</p>
                    {{?}}
                    {{?value.danOrShuang != 1 && value.danOrShuang != 2}}
                    <p class="vjs" js='{{=value.teacherId}}'>{{=value.teacherName}}</p>
                    {{?}}
                    <p class="vbq">{{=value.remarks}}</p></div>
                <span>{{=value.ypksCount}}/{{=value.zksCount}}</span></li>
            {{?}}
            {{~}}
        </script>

        <script type="text/template" id="kpjxblistTempByTeaId">
            {{~it:value:index}}
            {{?value.ypksCount<value.zksCount}}
            <li draggable="false" ondrop="drop(event,this)" ondragover="allowDrop(event)"
                ondragstart="drag(event, this)" teaId="{{=value.teacherId}}" jxbType="{{=value.type}}"
                jxbId="{{=value.id}}" ypksCnt="{{=value.ypksCount}}" zksCnt="{{=value.zksCount}}">
                <div class="ls"><p><em class="vxk">
                    {{?value.nickName == ""}}
                    {{=value.name}}
                    {{?}}
                    {{?value.nickName != ""}}
                    ...
                    <p class="vjs" js='{{=value.classroomId}}'>{{=value.classroomName}}</p>
...
<div class="fixedcont1" id="move1" style="width: 130px;">
    <div id="movebg1">点击可移动</div>
    <div class="info">
        <p class="p1 p4">
            <label style="float: left;margin-left: 15px;margin-top: 7px;">小课表</label>
...
<div class="container clearfix">
    ...
    <div class="tag">
        ...
        <div style="display: inline-block">
            <button class="b7" style="width: 94px;padding: 0;margin-left: 15px;background: #1ab02f" hidden>发布</button>
            <button class="b8" hidden>取消发布</button>
            <button class="fb" style="width: 94px;padding: 0;background: #7093bf">发布记录</button>
            <button class="b2" style="width: 94px;padding: 0;margin-left: 10px;background: #5851db">撤销</button>
            <button class="b3" style="width: 94px;padding: 0;background: #e45252">清空</button>
            <button class="b100" id='pkRule' style="background: #795548" >排课规则</button>
            <%--<button class="b99" style="background: #00a3e9">自动排课</button>--%>
            <button class="zdpk">自动排课</button>
            <button class="b10" id="tiaozheng">排课调整</button>
            <button class="b100" style="background: #c13584;border-radius: 4px;" id="swConflict">事务冲突</button>
        </div>
        <input type='hidden' id='aotuPk' value='${aotuPk}'/>
    ...
    <div>
        <div class="fl w195">
            <div class="f16 c-b">
                ...
                <input class="vt5" type="radio" name="na" id="teacher"><em class="mr10">教师</em>
            ...
            <script type="text/template" id="classrooms_templ">
                ...
                {{?value.className != ""}}
                <li ids="{{=value.roomId}}">
                    <span class="wha" style="min-height:30px!important;line-height:30px!important;" tname="{{=value.roomName}}">{{=value.roomName}}
                ...
                <li ids="{{=value.roomId}}">
                    <span class="wha" style="min-height:30px!important;line-height:30px!important;" tname="{{=value.roomName}}">{{=value.roomName}}
                    <label class="wla" ids="{{=value.roomId}}"></label></span>
                    <span class="f12" style="min-height:26px!important;line-height:26px!important;vertical-align: top;margin-top: -10px;"></span>
                </li>
                {{?}}
                {{~}}
            </script>
        ...
        <div class="fl w1000 clearfix" id="paikeByRoom">
            <div class="clearfix">
                <div class="fl ml10">
                    <div class="f16 c-b">课表-<span id="clsrom"></span><span id="kuohao">（</span><span id="ypks"></span><span id="fenge">/</span><span id="allks"></span>
                            ...
                            <table id="MyTable" class="tab2">
                                ...
                                <script type="text/template" id="weeks_temp">
                                    ...
                                    <th class="{{=value.roomId}}">
                                        ...
                                        <div class="zhou {{=value.id}}" style="width: 645px;">
                                    ...
                                    <td class="{{=it[0][0].classroomId}} gao" ids="{{=it[0][0].classroomId}}">
                                        ...
                                        <div class="zh{{=i+1}} ke">
                                            ...
                                            <div class="ke{{=value.y+1}} idx{{=value.x}}{{=value.y}} citt tb-clsrom"
                                                 ...
                                                 ondragstart="drag(event, this)" kbType="{{=value.kbType}}">
                                                {{?value.jxbId!=''}}
                                                    {{?value.kbType==1}}
                                                    <p class="zb pic"></p>
                                                    {{??value.kbType==2}}
                                                ...
                                                <div class="ls hls"><p class="whide"
            ...
            <%--</div>--%>
            <!--专项课-->
            <div class="zDv" style="margin-left: 10px;">
...
<div class="z-popup2 popup">
    <div class="popup-top">
        <em class="fl">事务冲突列表</em>
        <i class="close fr"></i>
    </div>
    <div class="tabDv">
        <div id="tabDv1">
            <p style="color: red;font-size: 20px;text-align: center;margin-left: -50px;">暂无冲突事务</p>
        </div>
        <div id="tabDv2">
            <table class="tab3" style="position: absolute">
                <thead>
                    <tr>
                        <th width="100">事务名称</th>
                        <th width="100">时间</th>
                        <th width="100">教学班</th>
                        <th width="100">老师</th>
                        <th width="100">教室</th>
            ...
            <div>
                <table class="tab3">
                    ...
                    <script type="text/template" id="conflict_temp">
                        {{~it:value:index}}
                        <tr>
                            <td>{{=value.desc}}</td>
                            <td>{{=value.xy}}</td>
...
<div class="zdpk-popup popup">
    <div class="popup-top">
        <em class="fl">自动排课</em>
        <i class="zdpk_close fr"></i>
    </div>
    <div class="zdpk-content">
        <button id="onlyNoZouBan" class="zb1">只排非走班课</button>
        <%--<button class="zb2">只排走班课</button>--%>
        <button class="zb3" id="zbfzb">走班与非走班</button>
        <%--<button class="zb4">重行政班模式</button>--%>
    </div>
</div>
