...
public final class CacheHandler {
	...
	//用户微服务存储
	public static final String MICRO_USER_KEY="ms:u:{0}";
	 ...
	 * @return
	 */
	public static SessionValue getMicroUserSessionValue(String userId)
	{
		String key=MessageFormat.format(MICRO_USER_KEY, userId);
		Map<String,String> m=RedisUtils.getMap(key);
		return new SessionValue(m);
	}
	 ...
	 * @param timeouts 单位秒
	 */
	public static void cacheMicroUserSessionValue(String userKey,SessionValue sv,int timeouts)
	{
		String key=MessageFormat.format(MICRO_USER_KEY, userKey);
		RedisUtils.cacheMap(key, sv.getMap(), timeouts);
	}
